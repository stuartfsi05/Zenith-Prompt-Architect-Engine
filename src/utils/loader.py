...
def load_system_prompt(filepath: str) -> str:
    ...
    Implements fallback logic to a sample prompt if the primary file is
    missing.
    ...
    if sample_path.exists():
        ...
        try:
            ...
            raise FileNotFoundError(
                f"Could not read sample prompt at {sample_path}"
            ) from e
    else:
        logger.critical(
            f"System prompt file not found. Checked: {primary_path} "
            f"and {sample_path}"
        )
        raise FileNotFoundError(
            "No system prompt file found. Please check configuration."
        )
