...
class Config:
    ...
    def load(cls) -> 'Config':
        ...
        try:
            ...
            logger.warning("Invalid TEMPERATURE value in .env. Defaulting to 0.7.")
            temperature = 0.7

        system_prompt_path = os.getenv("SYSTEM_PROMPT_PATH", "data/prompts/system_instruction.sample.md")
...
try:
    ...
    # but for simple access we can provide a lazy loader or just let main handle it.
    # For this architecture, we will let the main entry point or agent trigger the load
    # to ensure exception handling is done at the right level.
    pass
